...
'''
power on the pump: Serial.write("c".encode())
pwoer off the pump: Serial.write("d".encode())
...
class water():
    ...
    def power(self, onoff = 0):
        ...
        if(onoff==1):
            cmdSerial.write("c".encode())
        elif(onoff==0):
            cmdSerial.write("d".encode())

        if(self.debug==1):
            print("debug: power pump to ", onoff)
